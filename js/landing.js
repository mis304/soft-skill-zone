  ...
  function courseCard(c) {
    return (
      '<div class="card reveal">' +
        '<span class="badge">' + UI.esc(c.category) + "</span>" +
        "<h3>" + UI.esc(c.name) + "</h3>" +
        '<p class="outcome">' + UI.esc(c.outcome) + "</p>" +
        '<div class="card-meta">' +
          '<span class="dur">⏱ ' + UI.esc(c.duration) + "</span>" +
          '<span class="fee">' + UI.inr(c.fee) + "</span>" +
        "</div>" +
        '<a href="#contact" class="btn btn-ghost btn-block enquire-btn" data-course="' + UI.esc(c.name) + '">Enquire</a>' +
      "</div>"
    );
  }
